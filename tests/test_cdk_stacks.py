...
# --------------------------------------------------------------------------
# VPC e Endpoints (Req 14.7 - rede e conectividade)
# --------------------------------------------------------------------------
class TestVPC:
    """Testes para a VPC e seus endpoints."""

    def test_vpc_exists(self, template: Template):
        """VPC deve existir no stack."""
        template.resource_count_is("AWS::EC2::VPC", 1)

    def test_vpc_has_gateway_endpoints(self, template: Template):
        """VPC deve ter VPC Gateway Endpoints (S3, DynamoDB)."""
        ...
        assert len(gateway_endpoints) >= 2, (
            f"Esperado pelo menos 2 gateway endpoints (S3, DynamoDB), "
            f"encontrado {len(gateway_endpoints)}"
        )

    def test_vpc_has_interface_endpoints(self, template: Template):
        """VPC deve ter VPC Interface Endpoints."""
        resources = template.find_resources("AWS::EC2::VPCEndpoint")
        interface_endpoints = [
            r for r in resources.values()
            if r.get("Properties", {}).get("VpcEndpointType") == "Interface"
        ]
        # Esperamos ao menos: SecretsManager, SageMakerRuntime, SageMakerApi,
        # BedrockRuntime, CloudWatchLogs, StepFunctions
        assert len(interface_endpoints) >= 5, (
            f"Esperado pelo menos 5 interface endpoints, "
            f"encontrado {len(interface_endpoints)}"
        )

    def test_vpc_has_subnets(self, template: Template):
        """VPC deve ter subnets configuradas (Public, Private, Isolated)."""
        resources = template.find_resources("AWS::EC2::Subnet")
        # 3 tipos de subnet × 2 AZs = pelo menos 6 subnets
        assert len(resources) >= 6, (
            f"Esperado pelo menos 6 subnets (3 tipos × 2 AZs), "
            f"encontrado {len(resources)}"
        )

    def test_vpc_has_nat_gateway(self, template: Template):
        """VPC deve ter pelo menos 1 NAT Gateway."""
        resources = template.find_resources("AWS::EC2::NatGateway")
        assert len(resources) >= 1, "Esperado pelo menos 1 NAT Gateway"
